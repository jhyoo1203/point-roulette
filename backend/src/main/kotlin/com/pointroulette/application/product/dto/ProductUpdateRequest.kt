...
data class ProductUpdateRequest(
    ...
    @Schema(description = "상품명", example = "포인트 15,000원", required = false)
    val name: String? = null,

    @field:Min(value = 0, message = "가격은 0 이상이어야 합니다")
    @Schema(description = "가격", example = "15000", required = false)
    val price: Int? = null,

    @field:Min(value = 0, message = "재고는 0 이상이어야 합니다")
    @Schema(description = "재고", example = "150", required = false)
    val stock: Int? = null,

    @field:Size(max = 500, message = "설명은 500자 이하여야 합니다")
    @Schema(description = "상품 설명", example = "15,000 포인트 상품", required = false)
